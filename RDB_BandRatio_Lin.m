...
ft1 = fittype('a*x+b', 'indep', 'x');   % Linear regression: y = a*x+b
options1 = fitoptions('Method', 'NonlinearLeastSquares', 'Upper', [Inf 0]); 
[f1, S1] = fit(x, y, ft1, options1);

% Calibration parameters of the linear regression (unfiltered data)
a = f1.a;
b = f1.b;
yf1 = a*x+b;              % Fitted model
...
ft2 = fittype('a*x+b', 'indep', 'x');   % Linear regression: Y = a*X+b
options2 = fitoptions('Method', 'NonlinearLeastSquares', 'Upper', [Inf 0], 'StartPoint', [a b]);
[f2, S2] = fit(X, Y, ft2, options2);

% Calibration parameters of the linear regression (filtered data)
a_clean = f2.a;
b_clean = f2.b;
...
x = Depth_Val;
y = a_clean*z+b_clean;   % RDB (estimated bathymetry)
RDB = y;
...
Map_RDB = flip(a_clean*z+b_clean);
...
geotiffwrite('Map_SDB_lin', Map_RDB, R, CoordRefSysCode = Epsg);
...
if strcmp(Band_Ratio, 'Blue_Red') == 1
   title('RDB Calibration - Linear regression')
...
ylabel('Depth (m)')
legend('Data outliers', 'Data', 'Linear regression', 'Location', 'northwest', 'Fontsize', 10, 'Linewidth', 1)
dim = [.138 .64 .1 .1];
str1 = ['Linear regression:  y = a*x+b'];
str2 = ['a = ', num2str(a_clean, '%0.2f'), '     b = ', num2str(b_clean, '%0.2f')];
...
if strcmp(Band_Ratio, 'Blue_Red') == 1
   ...
   title('RDB Validation - Linear regression')
elseif strcmp(Band_Ratio, 'Blue_Green') == 1
